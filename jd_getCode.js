...
!(async () => {
    ...
    console.log(`东东农场助力码：${zl_ddnc}`)
    console.log(`京东萌宠助力码：${zl_jdmc}`)
    console.log(`种豆得豆助力码：${zl_zddd}`)
    console.log(`东东工厂助力码：${zl_jxgc}`)
    console.log(`京喜农场助力码：${zl_jxnc}`)
    console.log(`闪购盲盒助力码：${zl_sgmh}`)
   // console.log(`财富岛助力码：${zl_cfd}`)
    console.log(`签到领现金助力码：${zl_qdlxj}`)
    list += `export zl_ddnc="${zl_ddnc}"\n`
                                ...
                                $.taskVos.map((item) => {
                                    if (item.taskType === 14) {
                                       // console.log(`【京东账号${$.index}（${$.UserName}）东东工厂】${item.assistTaskDetailVo.taskToken}`);
                                        zl_ddgc +=`"${item.assistTaskDetailVo.taskToken}",`
                                    }
                                });
                            }
                        }
...
async function getJDFruit() {
    ...
    async function jdFruit() {
        ...
        if ($.farmInfo.farmUserPro) {
            ...
            zl_ddnc += `${$.farmInfo.farmUserPro.shareCode}@`;
        } else {
            /*console.log(
            ...
            $.post(url, async (err, resp, data) => {
                try {
                    ...
                    if (data.data.bizCode === 0) {
                        ...
                        zl_sgmh += `${invites && invites[0]['assistTaskDetailVo']['taskToken']}@`
                    }
                } catch (e) {
                    $.logErr(e, resp);
                } finally {
                ...
                if (err) {
                    ...
                    if (safeGet(data)) {
                        ...
                        if(data.code===0 && data.data.result){
                            ...
                            zl_qdlxj +=`${data.data.result.inviteCode}@`
                        }
                    }
                }
            } catch (e) {
