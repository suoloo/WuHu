...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        shareCodes=process.env.zl_jdmc
...
function readShareCode() {
    return new Promise(async resolve => {
        $.get({url: `https://cdn.jsdelivr.net/gh/suoloo/Code@master/shareCode/jdmc.txt`, 'timeout': 10000}, (err, resp, data) => {
            try {
                if (err) {
...
function requireConfig() {
    return new Promise(resolve => {
        ...
        console.log(`您提供了${process.env.zl_jdmc.split('@').length}个账号的东东萌宠助力码\n`);
        resolve()
    })
}
function TotalBean() {
