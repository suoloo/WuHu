...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        shareCodes=process.env.zl_zddd
        if (cookiesArr[i]) {
            cookie = cookiesArr[i];
...
function readShareCode() {
    return new Promise(async resolve => {
        $.get({url: `https://cdn.jsdelivr.net/gh/suoloo/Code@master/shareCode/zddd.txt`, 'timeout': 10000}, (err, resp, data) => {
            try {
                if (err) {
                    ...
                    if (data) {
                        console.log(`作者的助力码放到您固定的互助码后面(不影响已有固定互助)`)
                    }
                }
            } catch (e) {
                $.logErr(e, resp)
...
function shareCodesFormat() {
    return new Promise(async resolve => {
        ...
        newShareCodes = [];
        if (shareCodes) {
            newShareCodes = shareCodes.split("@");
        } else {
         ...
         if (readShareCodeRes) {
             newShareCodes.push(...(readShareCodeRes.split('@')));
         }
...
function requireConfig() {
    return new Promise(resolve => {
        ...
        console.log(`您提供了${process.env.zl_zddd.split('@').length}个账号的种豆得豆助力码\n`);
        resolve()
    })
}
