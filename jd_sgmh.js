...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        shareCodes=process.env.zl_sgmh
        cookie = cookiesArr[i];
        if (cookie) {
...
function requireConfig() {
    return new Promise(async resolve => {
        ...
        console.log(`您提供了${process.env.zl_sgmh.split("@").length}个账号的${$.name}助力码\n`);
        resolve()
    })
}
...
function readShareCode() {
    ...
    return new Promise(async resolve => {
        $.get({
            url: `https://cdn.jsdelivr.net/gh/suoloo/Code@master/shareCode/sgmh.txt`,
            'timeout': 10000
        }, (err, resp, data) => {
            try {
                if (err) {
