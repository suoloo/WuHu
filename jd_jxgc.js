...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        shareCodes=process.env.zl_jxgc
        if (cookiesArr[i]) {
            cookie = cookiesArr[i];
...
function readShareCode() {
    console.log(`开始`)
    return new Promise(async resolve => {
        $.get({url: `https://cdn.jsdelivr.net/gh/suoloo/Code@master/shareCode/jxgc.txt`, 'timeout': 10000}, (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
...
function requireConfig() {
    return new Promise(async resolve => {
        ...
        console.log(`您提供了${process.env.zl_jxgc.split("@").length}个账号的${$.name}助力码\n`);
        resolve()
    })
}
function TotalBean() {
